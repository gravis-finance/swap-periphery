...
// import { ChainId, ONE, TradeType, ZERO } from '@gravisswap/sdk/dist/constants'
// import { sortedInsert } from '@gravisswap/sdk/dist/utils'
// import { Currency, ETHER } from '@gravisswap/sdk/dist/entities/currency'
// import { CurrencyAmount } from '@gravisswap/sdk/dist/entities/fractions/currencyAmount'
// import { Fraction } from '@gravisswap/sdk/dist/entities/fractions/fraction'
// import { Percent } from '@gravisswap/sdk/dist/entities/fractions/percent'
// import { Price } from '@gravisswap/sdk/dist/entities/fractions/price'
// import { TokenAmount } from '@gravisswap/sdk/dist/entities/fractions/tokenAmount'
// import { Pair } from '@gravisswap/sdk/dist/entities/pair'
// import { Route } from '@gravisswap/sdk/dist/entities/route'
// import { currencyEquals, Token, WETH } from '@gravisswap/sdk/dist/entities/token'

// import { Trade } from '@gravisswap/sdk'
// const { bestTradeExactIn } = Trade
  ...
  for (const routerVersion of Object.keys(RouterVersion)) {
    ...
    describe('pre testing check before', () => {
      it('should be x pairs', async () => {
        expect(await factory.allPairsLength()).to.eq(6)
      })
    })
    ...
    describe(routerVersion, () => {
      ...
      it('addLiquidityETH with Fee:on', async () => {

        let feeAddress = '0x0000000000000000000000000000000000000FEE'
        await factory.setFeeTo(feeAddress)

        expect(await factory.feeTo()).to.eq(feeAddress, "Wrong fee address")
        ...
        let walletAddress = String(wallet.address);

        expect(await WETHPair.balanceOf(feeAddress)).to.eq(0)
        expect(await WETHPair.balanceOf(walletAddress)).to.eq(expectedLiquidity.sub(MINIMUM_LIQUIDITY))

        let pair = await getPair(provider, [wallet], factory, WETHPartner, WETH)
        ...
        expect(await WETHPair.balanceOf(feeAddress)).to.eq(0)
        expect(await WETHPair.balanceOf(walletAddress)).to.eq(0)
      })

      it('addLiquidityETH with Fee:on should fail', async () => {
      ...
      describe('swapExactTokensForTokens', () => {
        ...
        it('happy path, length path 2, 5 swap order', async () => {
        ...
        it('happy path, length path 4, 5 swap order', async () => {
        ...
        it('happy path, path length 4, 10 swap order', async () => {
